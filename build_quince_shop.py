...
<style>
  ...
  *{box-sizing:border-box}html{-webkit-text-size-adjust:100%}
  ...
  .fsub{font-size:.7rem;letter-spacing:.1em;text-transform:uppercase;color:var(--muted);margin:10px 0 2px;padding:0 4px}
...
function caratOpts(p){var out=[];(p.options||[]).forEach(function(o){if(o.name==="Carat"||o.name==="Center Carat Weight")out.push(o);});return out;}
function caratRank(s){var m=(""+s).match(/[\d.]+/);return m?parseFloat(m[0]):999;}
var CSEP="␟";  /* separates a carat option-name from its value in filter keys */
...
/* carat kept per option-name so "Carat" (total weight) and "Center Carat Weight" (centre stone) never conflate */
var CARAT_NAMES=(function(){var s=[];PRODUCTS.forEach(function(p){caratOpts(p).forEach(function(o){if(s.indexOf(o.name)<0)s.push(o.name);});});return s;})();
var CARAT_HAS={};CARAT_NAMES.forEach(function(n){CARAT_HAS[n]={};});
PRODUCTS.forEach(function(p){caratOpts(p).forEach(function(o){o.values.forEach(function(v){CARAT_HAS[o.name][v.label]=(CARAT_HAS[o.name][v.label]||0)+1;});});});
var HAS_CARAT={};CARAT_NAMES.forEach(function(n){Object.keys(CARAT_HAS[n]).forEach(function(v){HAS_CARAT[n+CSEP+v]=1;});});
var CARAT_LABEL=(CARAT_NAMES.length===1?CARAT_NAMES[0]:"Carat");
...
function matches(p){
  ...
  var kt=Object.keys(F.carat);if(kt.length){var okc=kt.some(function(key){var i=key.indexOf(CSEP);var nm=key.slice(0,i),val=key.slice(i+1);return caratOpts(p).some(function(o){return o.name===nm&&o.values.some(function(v){return v.label===val;});});});if(!okc)return false;}
...
function cardHTML(p,i){
  ...
  var disc=(mv.orig!=null&&m!=null&&mv.orig>m);
  var was=disc?'<span class="was">'+esc(money(mv.orig))+'</span>':'';
  var pct=disc?Math.round((mv.orig-m)/mv.orig*100):0;
...
function renderChips(){
  ...
  ["colors","carat","sizes","materials","prices"].forEach(function(k){Object.keys(F[k]).forEach(function(v){var disp=v;if(k==="carat"){var i=v.indexOf(CSEP);disp=(CARAT_NAMES.length>1?v.slice(0,i)+": ":"")+v.slice(i+1);}out.push('<span class="chip">'+esc(disp)+'<button data-ck="'+k+'" data-cv="'+esc(v)+'">✕</button></span>');});});
...
function groupOptions(kind){
  ...
  if(kind==="carat"){return CARAT_NAMES.map(function(nm){var vals=Object.keys(CARAT_HAS[nm]).sort(function(a,b){return caratRank(a)-caratRank(b);});var head=(CARAT_NAMES.length>1)?'<div class="fsub">'+esc(nm)+'</div>':'';return head+'<div class="fopts">'+vals.map(function(c){var key=nm+CSEP+c;return '<button class="fopt'+(F.carat[key]?" on":"")+'" data-group="carat" data-v="'+esc(key)+'"><span class="fbox"></span><span class="flab">'+esc(c)+'</span><span class="fcnt">'+CARAT_HAS[nm][c]+'</span></button>';}).join("")+'</div>';}).join("");}
  return "";
}
...
function openPDP(p,ci){
  LP=p;LSEL={};
  var co=colorOpt(p),v0;
  if(ci>=0&&co&&co.values[ci])v0=matchVar(p,{Color:co.values[ci].label});  /* swatch click -> that colour */
  else v0=minVar(p);                                                        /* card body -> cheapest, matches card 'from' + % off */
  for(var k in v0.sel)LSEL[k]=v0.sel[k];
